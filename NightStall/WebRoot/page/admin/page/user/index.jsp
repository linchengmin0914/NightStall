    ...
    table = $(".table-sort").dataTable({
        ...
        columns: [
			...
			},
            {
				...
				className : "nosort",
				render : function(data, type, full, meta) {
		        	var html = '<a title="密码重置" href="javascript:;" onclick="edit(\'用户密码重置\',\'${ctx}/admin/user/update.html?id=' + full.id + '\',\'1\',\'600\',\'200\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>';
		        	if(full.isAdmin == '0') {
...
function datadel() {
	...
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/sys/banner/delete/mul/done.html?ids=" + ids, function(data) {
			if(data.resCode == 1) {
				for(var i = 0;i< checkObjs.length; i++) {
					if($(checkObjs[i]).is(":checked")){ //jQuery方式判断
...
function del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/sys/banner/delete/done.html?id=" + id, function(data) {
			if(data.resCode == 1) {
...
function stop(obj,id){
	layer.confirm('确认要取消该用户的管理员权限吗？',function(index){
		ajaxForm("${ctx}/admin/user/update/done.html?id=" + id + "&isAdmin=0", function(data) {
			if(data.resCode == 1) {
				$('.search_btn').click();
...
function start(obj,id){
	layer.confirm('确认要将该用户设置为管理员吗？',function(index){
		ajaxForm("${ctx}/admin/user/update/done.html?id=" + id + "&isAdmin=1", function(data) {
			if(data.resCode == 1) {
				$('.search_btn').click();

...
<div class="pd-20" style="padding-top: 10px;">
	<div class="text-c col-12">
		<label class="form-label col-1" style="line-height: 30px;">商品名称：</label>
		<div class="formControls col-3">
			<input type="text" class="input-text" value="" placeholder="输入要查询的关键词" id="goodsName">
		</div>
		
		<label class="form-label col-1" style="line-height: 30px;">归属门店：</label>
		<div class="col-3">
		<span class="select-box">
			<select id="storeId" class="select" onchange="getCategoryByStoreId()">
	...
	<div class="text-c col-12" style="margin-top:10px;">
		<label class="form-label col-1" style="line-height: 30px;">归属分类：</label>
		<div class="col-3">
		<span class="select-box">
			<select id="cateId" class="select">
				
			</select>
		</span>
		</div>
	
		<label class="form-label col-1" style="line-height: 30px;">上架状态：</label>
		<div class="col-3">
		<span class="select-box">
			<select id="isSale" class="select">
				<option value="">所有</option>
	...
	<div class="cl pd-5 bg-1 bk-gray mt-10 mb-10 col-12"> 
		<span class="l">
			...
			<a href="javascript:;" onclick="dataSale()" class="btn btn-danger radius" style="background-color: green;border: none;"><i class="Hui-iconfont">&#xe615;</i> 批量上架</a> 
			<a href="javascript:;" onclick="dataNotSale()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe631;</i> 批量下架</a> 
			<a href="javascript:;" onclick="add('商品添加','${ctx}/admin/01/goods/add.html','1000','600')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加商品</a>
		</span> 
	</div>
...
$(function(){
    ...
    table = $(".table-sort").dataTable({
        ...
        ajax: function (data, callback, settings) {
            ...
            param.isSale = $('#isSale').val();
            param.cateId = $('#cateId').val();
            //ajax请求数据
            $.ajax({
                type: "POST",
                url: "${ctx}/admin/01/goods/list.html",
                cache: false,  //禁用缓存
                data: param,  //传入组装的参数
                dataType: "json",
        ...
        columns: [
			...
			{ "data": "storeNum" },
			{
				orderable: false,
				render : function(data, type, full, meta) {
					return '<a title="商品评价\" href="javascript:;" onclick="edit(\'商品评价\',\'${ctx}/admin/01/user/comment/index.html?goodsId=' + full.id + '\',\'1\',\'1000\',\'620\')" class="ml-5" style="text-decoration:none">查看评价</a>';
				}
			},
			{ "data": "storeName" },
			{
				...
				render : function(data, type, full, meta) {
					...
					if(full.isSale == '0') {
						return '<span class="label radius">下架</span>';
					}
            ...
            { "data": "createTime" },
            {
				...
				className : "nosort",
				render : function(data, type, full, meta) {
					html = '<a title="商品属性管理" href="javascript:;" onclick="edit(\'商品属性管理\',\'${ctx}/admin/01/goods/property/index.html?goodsId=' + full.id + '\',\'1\',\'1000\',\'620\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe60c;</i></a>';
					html += '<a title="图片管理\" href="javascript:;" onclick="edit(\'商品图片管理\',\'${ctx}/admin/01/goods/images/picture.html?goodsId=' + full.id + '\',\'1\',\'1000\',\'620\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe646;</i></a>';
					
		        	html += '<a title="编辑" href="javascript:;" onclick="edit(\'商品编辑\',\'${ctx}/admin/01/goods/update.html?id=' + full.id + '\',\'1\',\'1000\',\'600\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>' +
		        			'<a title="删除" href="javascript:;" onclick="del(this,\'' + full.id + '\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>';
		            
        			if(full.isSale == '0') {
						html += '<a style="text-decoration:none" onClick="start(this,\'' + full.id + '\')" href="javascript:;" title="标记为上架"><i class="Hui-iconfont">&#xe615;</i></a> ';
					} else {
						html += '<a style="text-decoration:none" onClick="stop(this,\'' + full.id + '\')" href="javascript:;" title="标记为下架"><i class="Hui-iconfont">&#xe631;</i></a>';
					}
...
function dataSale() {
	...
	var ids = arr.join(',');
	layer.confirm('确认要将所选的商品全部上架吗？',function(index){
		ajaxForm("${ctx}/admin/01/goods/sale/mul/done.html?ids=" + ids, function(data) {
			if(data.resCode == 1) {
				$('.search_btn').click();
...
function dataNotSale() {
	...
	var ids = arr.join(',');
	layer.confirm('确认要将所选的商品全部下架吗？',function(index){
		ajaxForm("${ctx}/admin/01/goods/notsale/mul/done.html?ids=" + ids, function(data) {
			if(data.resCode == 1) {
				$('.search_btn').click();
...
function stop2(obj,id){
	layer.confirm('确认要将该记录标记为不放置首页吗？',function(index){
...
function start2(obj,id){
	layer.confirm('确认要将该记录标记为放置首页吗？',function(index){

...
<div class="pd-20" style="padding-top: 10px;">
	<div class="text-c col-12">
		<label class="form-label col-1" style="line-height: 30px;">属性名称：</label>
	...
	<div class="cl pd-5 bg-1 bk-gray mt-10 mb-10 col-12"> 
		<span class="l">
			...
			<a href="javascript:;" onclick="add('属性添加','${ctx}/admin/01/goods/propertyname/add.html','700','350')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加属性</a>
		</span> 
	</div>
...
$(function(){
    ...
    table = $(".table-sort").dataTable({
        ...
        ajax: function (data, callback, settings) {
            ...
            $.ajax({
                type: "POST",
                url: "${ctx}/admin/01/goods/propertyname/list.html",
                cache: false,  //禁用缓存
                data: param,  //传入组装的参数
                dataType: "json",
        ...
        columns: [
            ...
            { "data": "createTime" },
            {
				...
				className : "nosort",
				render : function(data, type, full, meta) {
					html = '<a title="属性值管理" href="javascript:;" onclick="edit(\'属性值管理\',\'${ctx}/admin/01/goods/propertyvalue/index.html?propNameId=' + full.id + '\',\'1\',\'1000\',\'620\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe60c;</i></a>';
					
		        	html += '<a title="编辑" href="javascript:;" onclick="edit(\'属性编辑\',\'${ctx}/admin/01/goods/propertyname/update.html?id=' + full.id + '\',\'1\',\'700\',\'350\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>';
...
function datadel() {
	...
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/01/goods/propertyname/delete/mul/done.html?ids=" + ids, function(data) {
			if(data.resCode == 1) {
				for(var i = 0;i< checkObjs.length; i++) {
					if($(checkObjs[i]).is(":checked")){ //jQuery方式判断
...
function del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/01/goods/propertyname/delete/done.html?id=" + id, function(data) {
			if(data.resCode == 1) {
...
function stop(obj,id){
	layer.confirm('确认要将该记录标记为未启用吗？',function(index){
		ajaxForm("${ctx}/admin/01/goods/propertyname/update/done.html?id=" + id + "&status=0", function(data) {
			if(data.resCode == 1) {
				$('.search_btn').click();
...
function start(obj,id){
	layer.confirm('确认要将该记录标记为启用吗？',function(index){
		ajaxForm("${ctx}/admin/01/goods/propertyname/update/done.html?id=" + id + "&status=1", function(data) {
			if(data.resCode == 1) {
				$('.search_btn').click();

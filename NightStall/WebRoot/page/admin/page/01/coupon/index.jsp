...
<span class="c-gray en">&gt;</span> 优惠券管理 
<span class="c-gray en">&gt;</span> 优惠券列表 
...
<div class="pd-20" style="padding-top: 10px;">
	<div class="text-c col-12">
		<label class="form-label col-1" style="line-height: 30px;">优惠券名称：</label>
		<div class="formControls col-3">
			<input type="text" class="input-text" value="" placeholder="输入要查询的关键词" id="name">
		</div>
		
		<label class="form-label col-1" style="line-height: 30px;">可用范围：</label>
		<div class="col-3">
	...
	<div class="text-c col-12" style="margin-top:10px;">
		<label class="form-label col-1" style="line-height: 30px;">券类型：</label>
		<div class="col-3">
		<span class="select-box">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-10 mb-10 col-12"> 
		<span class="l">
			...
			<a href="javascript:;" onclick="add('优惠券添加','${ctx}/admin/01/coupon/add.html','1000','600')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加优惠券</a>
		</span> 
	</div>
	...
	<table class="table table-border table-bordered table-bg table-hover table-sort">
		<thead>
			<tr class="text-c">
				...
				<th width="50">券面额</th>
				<th width="50">消费门槛</th>
				<th width="50">优惠券库存</th>
				<th width="130">可用开始时间</th>
				<th width="130">可用结束时间</th>
				<th width="60">操作</th>
			</tr>
		</thead>
	</table>
...
$(function(){
    ...
    table = $(".table-sort").dataTable({
        ...
        ajax: function (data, callback, settings) {
            ...
            param.name = $('#name').val();
            param.scope = $('#scope').val();
            param.type = $('#type').val();
            //ajax请求数据
            $.ajax({
                type: "POST",
                url: "${ctx}/admin/01/coupon/list.html",
                cache: false,  //禁用缓存
                data: param,  //传入组装的参数
                dataType: "json",
        ...
        columns: [
            ...
            { "data": "name" },
			{
				...
				render : function(data, type, full, meta) {
					if(full.scope == 1) {
						return "<span>全场通用</span>";
					} else if(full.scope == 3) {
						return "<span>指定门店</span>";
					} else if(full.scope == 2) {
						return "<span>指定商品</span>";
					} else {
						return "<span>指向私发</span>";
					}
            ...
            { "data": "validend" },
            {
				...
				className : "nosort",
				render : function(data, type, full, meta) {
		        	var html = '<a title="编辑" href="javascript:;" onclick="edit(\'优惠券编辑\',\'${ctx}/admin/01/coupon/update.html?id=' + full.id + '\',\'1\',\'1000\',\'600\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>' +
...
function datadel() {
	...
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/01/coupon/delete/mul/done.html?ids=" + ids, function(data) {
			if(data.resCode == 1) {
				for(var i = 0;i< checkObjs.length; i++) {
					if($(checkObjs[i]).is(":checked")){ //jQuery方式判断
...
function del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/01/coupon/delete/done.html?id=" + id, function(data) {
			if(data.resCode == 1) {

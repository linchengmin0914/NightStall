...
<span class="c-gray en">&gt;</span> 运费模板管理 
...
<div class="pd-20" style="padding-top: 10px;">
	<div class="text-c col-12">
		<label class="form-label col-1" style="line-height: 30px;">运费模板名称：</label>
		<div class="formControls col-4">
			<input type="text" class="input-text" value="" placeholder="输入运费模板名称" id="name">
		</div>
		<div class="col-1">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-10 mb-10 col-12"> 
		<span class="l">
			...
			<a href="javascript:;" onclick="add('添加运费模板','${ctx}/admin/01/postage/add.html','800','550')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加运费模板</a>
		</span> 
	</div>
...
$(function(){
    ...
    table = $(".table-sort").dataTable({
        ...
        ajax: function (data, callback, settings) {
            ...
            $.ajax({
                type: "POST",
                url: "${ctx}/admin/01/postage/list.html",
                cache: false,  //禁用缓存
                data: param,  //传入组装的参数
                dataType: "json",
        ...
        columns: [
            ...
            { "data": "createTime" },
            {
				...
				className : "nosort",
				render : function(data, type, full, meta) {
		        	var html = '<a title="编辑" href="javascript:;" onclick="edit(\'运费模板编辑\',\'${ctx}/admin/01/postage/update.html?id=' + full.id + '\',\'1\',\'800\',\'500\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>' +
...
function datadel() {
	...
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/01/postage/delete/mul/done.html?ids=" + ids, function(data) {
			if(data.resCode == 1) {
				for(var i = 0;i< checkObjs.length; i++) {
					if($(checkObjs[i]).is(":checked")){ //jQuery方式判断
...
function del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/01/postage/delete/done.html?id=" + id, function(data) {
			if(data.resCode == 1) {
...
function stop(obj,id){
	layer.confirm('确认要将该记录标记为未启用吗？',function(index){
		ajaxForm("${ctx}/admin/01/postage/openclose/done.html?id=" + id + "&isOpen=0", function(data) {
			if(data.resCode == 1) {
				$('.search_btn').click();
...
function start(obj,id){
	layer.confirm('确认要将该记录标记为启用吗？',function(index){
		ajaxForm("${ctx}/admin/01/postage/openclose/done.html?id=" + id + "&isOpen=1", function(data) {
			if(data.resCode == 1) {
				$('.search_btn').click();

...
<span class="c-gray en">&gt;</span> 属性值管理 
<span class="c-gray en">&gt;</span> 属性值列表 
...
<div class="pd-20" style="padding-top: 10px;">
	<div class="text-c col-12">
		<label class="form-label col-" style="line-height: 30px;">属性值：</label>
		<div class="formControls col-3">
			<input type="text" class="input-text" value="" placeholder="输入要查询的关键词" id="title">
		</div>
	...
	<div class="cl pd-5 bg-1 bk-gray mt-10 mb-10 col-12"> 
		<span class="l">
			...
			<a href="javascript:;" onclick="add('属性值添加','${ctx}/admin/01/goods/propertyvalue/add.html?propNameId=${propNameId}','700','400')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加属性值</a>
		</span> 
	</div>
	
	<input type="hidden" id="propNameId" value="${propNameId}"/>
	<table class="table table-border table-bordered table-bg table-hover table-sort">
		<thead>
			<tr class="text-c">
				<th width="25"><input type="checkbox" name="" value=""></th>
				<th width="40">ID</th>
				<th width="150">属性值名</th>
				<th width="100">属性值</th>
				<th width="50">排序序号</th>
				<th width="50">状态</th>
...
$(function(){
    ...
    table = $(".table-sort").dataTable({
        ...
        ajax: function (data, callback, settings) {
            ...
            console.log($('#propNameId').val());
            param.propNameId = $('#propNameId').val();
            //ajax请求数据
            $.ajax({
                type: "POST",
                url: "${ctx}/admin/01/goods/propertyvalue/list.html",
                cache: false,  //禁用缓存
                data: param,  //传入组装的参数
                dataType: "json",
        ...
        columns: [
            ...
            { "data": "sort" },
			{
				...
				render : function(data, type, full, meta) {
					...
					if(full.status == '0') {
						return '<span class="label radius">未启用</span>';
					}
				}
			},
            { "data": "createTime" },
            {
				...
				className : "nosort",
				render : function(data, type, full, meta) {
		        	html = '<a title="编辑" href="javascript:;" onclick="edit(\'属性值编辑\',\'${ctx}/admin/01/goods/propertyvalue/update.html?id=' + full.id + '\',\'1\',\'700\',\'400\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>';
		        	
		        	if(full.id != 1) {
			        	html = '<a title="删除" href="javascript:;" onclick="del(this,\'' + full.id + '\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>';
			        	if(full.status == '0') {
...
function datadel() {
	...
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/01/goods/propertyvalue/delete/mul/done.html?ids=" + ids, function(data) {
			if(data.resCode == 1) {
				for(var i = 0;i< checkObjs.length; i++) {
					if($(checkObjs[i]).is(":checked")){ //jQuery方式判断
...
function del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/01/goods/propertyvalue/delete/done.html?id=" + id, function(data) {
			if(data.resCode == 1) {
...
function stop(obj,id){
	layer.confirm('确认要将该记录标记为未启用吗？',function(index){
		ajaxForm("${ctx}/admin/01/goods/propertyvalue/update/done.html?id=" + id + "&status=0", function(data) {
			if(data.resCode == 1) {
				$('.search_btn').click();
...
function start(obj,id){
	layer.confirm('确认要将该记录标记为启用吗？',function(index){
		ajaxForm("${ctx}/admin/01/goods/propertyvalue/update/done.html?id=" + id + "&status=1", function(data) {
			if(data.resCode == 1) {
				$('.search_btn').click();

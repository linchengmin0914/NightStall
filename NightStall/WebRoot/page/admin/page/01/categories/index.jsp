...
<div class="pd-20" style="padding-top: 10px;">
	<div class="text-c col-12">
		<label class="form-label col-1" style="line-height: 30px;">分类名称：</label>
		<div class="formControls col-3">
			<input type="text" class="input-text" value="" placeholder="输入要查询的关键词" id="cateName">
		</div>
		
		<label class="form-label col-1" style="line-height: 30px;">归属门店：</label>
		<div class="col-3">
		<span class="select-box">
			<select id="storeId" class="select">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-10 mb-10 col-12"> 
		<span class="l">
			...
			<a href="javascript:;" onclick="add('商品分类添加','${ctx}/admin/01/goods/categories/add.html','700','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加分类</a>
		</span> 
	</div>
...
$(function(){
    ...
    table = $(".table-sort").dataTable({
        ...
        ajax: function (data, callback, settings) {
            ...
            param.cateName = $('#cateName').val();
            param.storeId = $('#storeId').val();
            //ajax请求数据
            $.ajax({
                type: "POST",
                url: "${ctx}/admin/01/goods/categories/list.html",
                cache: false,  //禁用缓存
                data: param,  //传入组装的参数
                dataType: "json",
        ...
        columns: [
            ...
            { "data": "createTime" },
            {
				...
				className : "nosort",
				render : function(data, type, full, meta) {
		        	html = '<a title="编辑" href="javascript:;" onclick="edit(\'商品分类编辑\',\'${ctx}/admin/01/goods/categories/update.html?id=' + full.id + '\',\'1\',\'700\',\'500\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>' +
...
function datadel() {
	...
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/01/goods/categories/delete/mul/done.html?ids=" + ids, function(data) {
			if(data.resCode == 1) {
				for(var i = 0;i< checkObjs.length; i++) {
					if($(checkObjs[i]).is(":checked")){ //jQuery方式判断
...
function del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/01/goods/categories/delete/done.html?id=" + id, function(data) {
			if(data.resCode == 1) {

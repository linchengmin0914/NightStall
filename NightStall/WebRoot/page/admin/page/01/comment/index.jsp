...
<span class="c-gray en">&gt;</span> 商品评价管理 
...
<div class="pd-20" style="padding-top: 10px;">
	<div class="text-c col-12">
		<label class="form-label col-1" style="line-height: 30px;">关键词：</label>
		<div class="formControls col-3">
			<input type="text" class="input-text" value="" placeholder="输入如商品名称、订单号、评价内容关键词" id="key">
		</div>
	...
	<div class="cl pd-5 bg-1 bk-gray mt-10 mb-10 col-12"> 
		<span class="l">
			...
			<a href="javascript:;" onclick="add('评论添加','${ctx}/admin/01/user/comment/add.html?goodsId=${goodsId}','700','450')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加评论</a>
		</span> 
	</div>
...
$(function(){
    ...
    table = $(".table-sort").dataTable({
        ...
        ajax: function (data, callback, settings) {
            ...
            $.ajax({
                type: "POST",
                url: "${ctx}/admin/01/user/comment/list.html",
                cache: false,  //禁用缓存
                data: param,  //传入组装的参数
                dataType: "json",
        ...
        columns: [
            ...
            { "data": "createTime" },
            {
				...
				className : "nosort",
				render : function(data, type, full, meta) {
		        	html = '<a title="编辑" href="javascript:;" onclick="edit(\'商品评价编辑\',\'${ctx}/admin/01/user/comment/update.html?id=' + full.id + '\',\'1\',\'700\',\'450\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>' +
...
function datadel() {
	...
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/01/user/comment/delete/mul/done.html?ids=" + ids, function(data) {
			if(data.resCode == 1) {
				for(var i = 0;i< checkObjs.length; i++) {
					if($(checkObjs[i]).is(":checked")){ //jQuery方式判断
...
function del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/01/user/comment/delete/done.html?id=" + id, function(data) {
			if(data.resCode == 1) {

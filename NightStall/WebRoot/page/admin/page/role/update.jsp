	...
	<form action="" method="post" class="form form-horizontal" id="form-add">
		<div class="row cl">
			...
			<div class="formControls col-8">
				<input type="text" class="input-text" value="${entity.name}" placeholder="" id="name" name="name" datatype="*1-100" nullmsg="名称不能为空！" >
			</div>
			<div class="col-2"> </div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-8">
				<textarea id="des" name="des" cols="" rows="" class="textarea"  placeholder="请输入..." dragonfly="true" onKeyUp="textarealength(this,1000)">${entity.des}</textarea>
		...
		<div class="row cl">
			...
			<div class="formControls col-10">
				...
				<dl class="permission-list">
					<dt>
						<label>
							<input type="checkbox" value="${map.key.id}" name="rightId" class="right1 right" <c:if test="${rightIds.contains(map.key.id)}">checked="checked"</c:if>>
							${map.key.name}</label>
					</dt>
					<dd>
						<dl class="cl permission-list2">
							...
							<dt style="width: auto;padding: 5px;">
								<label class="">
									<input type="checkbox" value="${right.id}" name="rightId" class="right2 right" <c:if test="${rightIds.contains(right.id)}">checked="checked"</c:if>>
									${right.name}</label>
							</dt>
							</c:forEach>
						</dl>
	...
	$(".right2").click(function(){
	 ...
	 $("#form-add").Validform({  
        ...
        beforeSubmit:function(){  
            ...
            ajaxForm("${ctx}/admin/role/update/done.html?rightIds=" + rightIds, function(data) {
				if(data.resCode == 1) {

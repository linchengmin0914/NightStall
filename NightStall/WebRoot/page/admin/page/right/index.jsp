...
<div class="pd-20" style="padding-top: 10px;">
	<div class="text-c">
		<label class="form-label col-1" style="line-height: 30px;">权限名称：</label>
		<div class="formControls col-4">
			<input type="text" class="input-text" value="" placeholder="输入权限名称" id="name">
		</div>
		<label class="form-label col-1" style="line-height: 30px;">归属类别：</label>
		<div class="col-4">
		<span class="select-box">
			<select id="parentId" class="select">
				<option value="">所有</option>
				<option value="parentId">根级</option>
		...
		<button type="button" class="btn btn-success" onclick="search();"><i class="Hui-iconfont">&#xe665;</i> 搜权限</button>
	...
	<div class="cl pd-5 bg-1 bk-gray mt-10 mb-10"> 
		<span class="l">
			...
			<a href="javascript:;" onclick="add('添加权限','${ctx}/admin/right/add.html','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加权限</a>
		</span> 
	</div>
    ...
    table = $(".table-sort").dataTable({
        ...
        columns: [
            ...
            { "data": "createTime" },
            {
				...
				className : "nosort",
				render : function(data, type, full, meta) {
		        	html = '<a title="编辑" href="javascript:;" onclick="edit(\'权限编辑\',\'${ctx}/admin/right/update.html?id=' + full.id + '\',\'1\',\'800\',\'500\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>' +
...
function datadel() {
	...
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/right/delete/mul/done.html?ids=" + ids, function(data) {
			if(data.resCode == 1) {
				for(var i = 0;i< checkObjs.length; i++) {
					if($(checkObjs[i]).is(":checked")){ //jQuery方式判断
...
function del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		ajaxForm("${ctx}/admin/right/delete/done.html?id=" + id, function(data) {
			if(data.resCode == 1) {
